...
class ClassifierTrainer:
    def __init__(
        ...
        """
        architecture: "swin"
        model_name: see classifier_swin.py for timm output
        """

        self.data_dir = data_dir
        self.architecture = model.architecture  # swin
    ...
    def create_plot(self, history, epochs_run, note, params_str):
        ...
        filename = (
            results_dir
            / f"{self.architecture}_{self.model_name}_{note}_{timestamp}.png"
        )
...
if __name__ == "__main__":
    model_0 = SwinClassifier(pretrained=True, model_name="swin_tiny_patch4_window7_224")

    optimizer_0 = optim.AdamW(model_0.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    trainer_scratch_optimized_0.train(
        epochs=150,
        note="swin_tiny_standard_finetune",
    )
    ...
    model_1 = SwinClassifier(pretrained=True, model_name="swin_tiny_patch4_window7_224")
    ...
    optimizer_1 = optim.AdamW(
        [
            {"params": decay_1, "weight_decay": 5e-4},
            {"params": no_decay_1, "weight_decay": 0.0},
        ],
        lr= 1e-4
    )
    ...
    trainer_scratch_optimized_1.train(
        epochs= 150,
        note= "swin_tiny_optimized_weight_decay"
    )
