...
```
.
├── config.yaml                         # pipeline routing + all hyperparameters -> change this to your preferred settings / paths 
├── inference.py                        # provided and adapted interface script — wired to YoloWorld + ConvNeXt/GCViT + optional OOD gate
├── requirements.txt
├── README.md
...
│   ├── inference_provided.py           # same, against the provided dataset (runs YoloWorld detection first)
...
└── animal_recognition/
    ...
    │   └── weights/                    # main weights folder -> unzip the provided weights here
    ├── data/
    ...
    └── src/
        ...
        │   ├── augmentations.py / _mild.py / _stronger.py / _vetted.py   # augmentation presets, only stronger and vetted used during training 
        │   ├── downloader_reddit.py    # gallery-dl scraper for breed subreddits (needs Firefox/Browser session cookie)
        │   ├── downloader_tiger_cat.py # streams the Tiger Cat class from ImageNet-1k 
        │   ├── downloader_test_dataset.py  # one-off: built the 100-img/breed held-out test set from Oxford Pets/Stanford Dogs/ImageNet-1k
        │   └── sanitize_scraped_data.py    # runs YoloWorld over raw/ to split scraped images into processed/accepted and processed/rejected
        ├── models/
        │   ├── yoloworld.py             # YoloWorldDetector — detector used in inference.py 
        │   ├── classifier_convnext.py   # ConvNextClassifier — used in the pipeline
        │   ├── classifier_gcvit.py      # GCViTClassifier  — used in the pipeline
        │   ├── yolo.py                  # closed-vocabulary YOLO detector - evaluated but not used 
        │   ├── classifier_resnet50.py   # implemented for reference, not trained or used 
        │   └── classifier_swin.py       # Swin classifier -  not part of the main pipeline
        ├── training/
        │   ├── train_classifier.py             # main parameterized trainer (schedulers, augmentation presets, etc.)
        │   ├── train_classifier_top_5.py / _2nd_attempt.py   # experiments narrowing to the 5 best configs with different scaling formulas
        ...
        ├── evaluation/
        │   ├── inference_batch.py       # multi-model leaderboard (requires Tkinter file picker) - detects gcvit/convnextv2 checkpoints by filename
        ...
        └── xai/                          
            ├── run_xai.py                # CLI entry point, supports convnext / gcvit / swin via a registry
...
```bash
python3.11 -m venv venv #requires Python 3.11 on mac, otherwise numpy/scipy wheels fail to build
...
`inference_batch.py` additionally needs a Python built with Tk support (`import tkinter` must work) — it opens a native file picker. 
...
# all other values are best changed in config.yaml
```
